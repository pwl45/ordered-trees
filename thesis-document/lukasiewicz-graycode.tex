...
Note that $2$, $30$ and $3$ cannot be suffixes of a Lukasiewicz word. Since Lukasiewicz words have sums equal to their length, if a Lukasiewicz word of length $n$ ended with $3$ its $n-1$st prefix would have length $n-1$ and sum $n-3$, which would make it an invalid Lukasiewicz word. Similarly, if a Lukasiewicz word ended with $30$ or $2$, its $n-1$st prefix would have length $n-1$ and sum $n-2$, which would make it invalid by the same principle.  More generally, a Lukasiewicz word cannot have a scut with a sum greater than its length.

The rightmost column of Figure \ref{fig:LukaTable} illustrates this recursive ordering for Lukasiewicz words with content \{0,0,0,1,2,3\}.
