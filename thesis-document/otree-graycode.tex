...
Fig. \ref{fig:exampleotree} gives an example illustrating an ordered tree's first branching and the nodes $O,P,G,$ and $L$. In the special case that. if $\tree{T}$ has no first branching, and therefore $\tree{T}$ is a path, we define $O$ to be $\tree{T}$'s unique leaf.
...
\begin{subnumcases}{\nextTree{T} = \label{eq:otreeRule_cyclic}}
    % \poppush{P}{root} &  $\leftdown{T}=\tree{T}$ \label{eq:otree_noo_cyclic}\\
...
% \begin{subnumcases}{\nextTree{T} = \label{eq:otreeRule_cyclic}}
%     \treeshift{T}{F}{root} & $\leftdown{T}=\tree{T}$ \label{eq:otree_noo_cyclic}\\
%     \treeshift{\treeshift{T}{L}{G}}{O}{root}& if $P \ne root $ and O has no children \label{eq:otree_zeroshift_cyclic}\\
...
\end{tikzpicture}
	...
	\end{center}
	\caption{$\tree{T}$ has no first branching \\
	$\poppush{P}{root}$ \\
...
%     \caption{Our pull Gray code is generated by the two case successor rule in \eqref{eq:next}.
...
This section will prove that the successsor rule does in fact in \ref{eq:otreeRule} generates all ordered trees with $n$ nodes.  
\subsection{Remarks}
...
\begin{subnumcases}{\nextTree{T} = \label{eq:expandedOtree}}
    \poppush{P}{root}  & $\leftdown{T}=\tree{T}$ \label{eq:ex_otree_n}\\
...
\begin{proof}
    ...
    % Note that the conditions for \ref{eq:ex_otree_k1_1} and \ref{eq:expandedDyck_k1_1} are equivalent, as $O$ has at least 1 child $\iff D_{x+1}=1$.
